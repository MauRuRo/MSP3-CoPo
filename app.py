...
# This extension is necessary to be able to use 'global'
# variables in for loops in the jinja html on the creations pages.
...
# homepage; the authoruser variable is set to None;
# this will result in the condition for the
# searchFuncAuthor not being satisfied;
# thus not performing a author search on page load complete.
@app.route('/creations')
def creations():
...
# homepage/authorsearch; from the poem page the user can navigate
# to a search of an author by clicking on their name which has
# an href. It puts the author name in to a hidden div; from their
# jquery pulls the author name to perform an Ajax action with it
...
# homepage/theme selection: performs a query on the
# database and returns all titles with selected theme, sorted.
...
# homepage/author selection: performs a query on
# the database and returns all titles with selected author, sorted.
...
# search titles: performs a query to the database for a
# partial match with titles of the input in the searchbar.
...
# Create: renders the 'create' page when create button is clicked:
# the themes have to be pulled from the database as these may vary over time.
@app.route('/create')
def create():
    return render_template("create.html", copo_themes=mongo.db.copo_themes.find().sort("theme", 1))


# Collaborate: renders the collaborate page with all the info of
# the selected poem pulled from the database and pre-inputted in to the form.
...
# Update: uploads the adjusted poem on submission of
# the collaboration form and also updates the version history information.
...
# Upload Poem: uploads new poem to the database and (if applicable)
# uploads new user to the database after submission of create form.
...
# when filling in the user info fields on the forms.
